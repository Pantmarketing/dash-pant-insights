...
const Dashboard = () => {
  ...
  const totals = useMemo(() => {
    const sum = filtered.reduce(
      ...
      {
        investimento: 0, impressoes: 0, cliques: 0, visualizacoes_pagina: 0,
        leads: 0, adicoes_carrinho: 0, inicios_checkout: 0, compras: 0, faturamento: 0,
      }
    );
    ...
    const ROI =
      sum.investimento + operationalCosts > 0
        ? (sum.faturamento - sum.investimento - operationalCosts) /
          (sum.investimento + operationalCosts)
        : 0;
  ...
  const barDataLead = useMemo(() => {
    ...
    filtered.forEach((r) => {
      byCamp[r.nome_campanha] =
        (byCamp[r.nome_campanha] ?? 0) + (r.leads ?? 0);
    });
    return Object.entries(byCamp).map(([nome_campanha, leads]) => ({
      nome_campanha,
      leads,
    }));
  }, [filtered]);
  ...
  const barDataEcom = useMemo(() => {
    ...
    filtered.forEach((r) => {
      byCamp[r.nome_campanha] =
        (byCamp[r.nome_campanha] ?? 0) + (r.faturamento ?? 0);
    });
    return Object.entries(byCamp).map(([nome_campanha, faturamento]) => ({
      nome_campanha,
      faturamento,
    }));
  }, [filtered]);
  ...
  // Configs dos gráficos (cores/labels)
  const leadConfig = {
    leads: { label: "Leads", color: "hsl(var(--chart-1))" },
    cpl: { label: "CPL", color: "hsl(var(--chart-2))" },
  } as const;

  const ecomLineConfig = {
    faturamento: { label: "Faturamento", color: "hsl(var(--chart-1))" },
    roas: { label: "ROAS", color: "hsl(var(--chart-2))" },
  } as const;

  const barLeadConfig = { leads: { label: "Leads", color: "hsl(var(--chart-1))" } } as const;
  const barEcomConfig = { faturamento: { label: "Faturamento", color: "hsl(var(--chart-1))" } } as const;
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <Card>
              ...
              <CardContent className="h-72">
                <ChartContainer config={leadConfig} className="w-full h-full">
                  <LineChart data={lineDataLead}>
                    ...
                    <Line yAxisId="left" type="monotone" dataKey="leads" stroke="var(--color-leads)" dot={false} />
                    <Line yAxisId="right" type="monotone" dataKey="cpl" stroke="var(--color-cpl)" dot={false} />
                  </LineChart>
                </ChartContainer>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="h-72">
                <ChartContainer config={barLeadConfig} className="w-full h-full">
                    ...
                    {filtered.map((r, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell className="text-right">
                          {r.investimento.toLocaleString(undefined, { style: "currency", currency: "BRL" })}
                        </TableCell>
        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <Card>
              ...
              <CardContent className="h-72">
                <ChartContainer config={ecomLineConfig} className="w-full h-full">
                  <LineChart data={lineDataEcom}>
                    ...
                    <YAxis yAxisId="right" orientation="right" />
                    <ChartTooltip content={<ChartTooltipContent />} />
                    <Line yAxisId="left" type="monotone" dataKey="faturamento" stroke="var(--color-faturamento)" dot={false} />
                    <Line yAxisId="right" type="monotone" dataKey="roas" stroke="var(--color-roas)" dot={false} />
                  </LineChart>
                </ChartContainer>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="h-72">
                <ChartContainer config={barEcomConfig} className="w-full h-full">
                  ...
                  <TableBody>
                    {filtered.map((r, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell className="text-right">
                          {(r.faturamento ?? 0).toLocaleString(undefined, { style: "currency", currency: "BRL" })}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
